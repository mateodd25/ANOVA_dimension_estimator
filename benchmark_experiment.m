...
for jj = 1:nb_manifolds
    ...
    for ii = 1:T
        ...
        [d_angular_naive, d_angular_heuristic_naive ] = ANOVA_global_estimator(X, 'maxdimension',100); %We set the maximum dimension high so that we don't have to train the kernel-based estimator multiple times
        [d_angular, d_angular_heuristic ] = ANOVA_global_estimator(X, 'naive', 0, 'maxdimension', 100);
        d_LB = 0;
        for kk = 10:20
...
%          [d_danco,~] = DANCo(X,'inds',inds,'dists',dists);
        estimated_dim(:, jj) = estimated_dim(:, jj) + [d_angular_naive; d_angular_heuristic_naive; d_angular; d_angular_heuristic; d_LB; d_danco];
        MPE(:, jj) = MPE(:, jj) + abs([d_angular_naive; d_angular_heuristic_naive; d_angular; d_angular_heuristic; d_LB; d_danco] - d_correct)/d_correct;  
        MSE(:, jj) = MSE(:, jj) + abs([d_angular_naive; d_angular_heuristic_naive; d_angular; d_angular_heuristic; d_LB; d_danco] - d_correct).^2;  
